...
void solid(uint32_t c) {
    ...
    strip.show(); // call show after making changes to make the changes appear
}
...
// does a rainbow wipe of the strip
...
void allRainbowCycle(uint8_t wait) {
  ...
  for(j=0; j<256*5; j++) { // 5 cycles of all colors on wheel
    ...
    for(i=0; i< numPixels; i++) {
      strip.setPixelColor(i, Wheel(((i * 256 / strip.numPixels()) + j) & 255));
      monitorStrip.setPixelColor(i, Wheel(((i * 256 / monitorStrip.numPixels()) + j) & 255));
    }
    strip.show();
    monitorStrip.show();
    delay(wait);
